      ...
      <div ref={dialogRef} className="relative mx-auto mt-10 w-[92%] max-w-3xl rounded-xl border border-white/10 bg-gray-900/95 text-white shadow-2xl p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-300">View contact submissions stored in MongoDB.</p>
          </div>
            ...
            {messages.map((m, idx) => (
              <div key={(m.id || idx) as any} className="py-4">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <div className="text-xs text-gray-400">{m.created_at || m.createdAt ? new Date(m.created_at || m.createdAt || '').toLocaleString() : ''}</div>
                </div>
