...
export default function MacTerminal({ isOpen, onClose }: MacTerminalProps) {
  ...
  const hasBirthYear = userConfig.yearOfBirth > 1900;
  const computedAge = hasBirthYear ? currentDate.getFullYear() - userConfig.yearOfBirth : null;
  const terminalLabel = userConfig.website.replace(/^https?:\/\//, '').replace(/\/$/, '');
...
Q: "How old are you?"
A: "${hasBirthYear ? `I'm ${computedAge} years old` : 'My resume does not list my age; professionally, I am a BCA graduate and full stack developer.'}"

Core details about me:
- I live in ${userConfig.location}
- I'm a ${userConfig.role}
- My email is ${userConfig.contact.email}
- My phone number is ${userConfig.contact.phone}
- My LinkedIn is ${userConfig.social.linkedin}
- My GitHub is ${userConfig.social.github}
${hasBirthYear ? `- I was born in ${userConfig.yearOfBirth}` : '- My resume does not list my birth year'}
      ...
      <div className='p-1 text-gray-200 font-mono text-sm h-full flex flex-col overflow-hidden'>
          ...
          {chatHistory.messages.map((msg, index) => (
            <div key={index} className='mb-2'>
              {msg.role === 'user' ? (
                ...
                <div className='flex items-start space-x-2'>
                  <span className='text-green-400 font-bold'>${terminalLabel}</span>
                  <pre className='whitespace-pre-wrap'>{msg.content}</pre>
                </div>
              )}
            </div>
        ...
        <form onSubmit={handleSubmit} className='mt-2 rounded-lg p-2'>
          <div className='flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-2'>
            <span className='whitespace-nowrap text-green-400 font-bold'>{terminalLabel} root %</span>
            <input
              type='text'
